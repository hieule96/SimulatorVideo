...
u32 H265frametype(unsigned char *p){
  u32 nal = p[13] & 31, turns = 0;
...
static int enqueue_v(void *context)
{
  ...
  if (ctx->rtp->M) {
    switch (ctx->sub_type) {
      ...
      case 'HEVC': ctx->frametype = H264frametype(p);break;
        ...
        if (!ev) {
          ...
          if (rtp_video.M) {
            switch (video_sub_type) {
              ...
              case 'hvc1': video_ft = H264frametype(p); break;
              default    : video_ft = 4; break;
            }
          }
